...
} from "@/stores/modifications-store.ts";
...
const bodyTypeOptions = [
    ...
    { label: "Пикап", value: "Пикап" },
    {
        ...
        value: "Микроавтобус / пассажирский",
    },
...
function updateForm(item: IModification) {
    ...
    if (form.value.eav_attributes.vinCode != null) {
        form.value.eav_attributes.vinCode =
            form.value.eav_attributes.vinCode!.toString();
    }
}
...
onMounted(() => {
    ...
    if (route.query.modificationId != null) {
        modificationStore
            .loadModification(parseInt(route.query.modificationId!.toString()))
            .then((res: any) => {
                updateForm(res);
            });
    }
